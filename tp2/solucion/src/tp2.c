...
//devuelve la cantidad de ciclos promedio por llamada a filtro_color_c/asm
float aplicar_filtro_color (const char *implementacion,
...
//devuelve la cantidad de ciclos promedio por llamada a filtro_miniature_c/asm
float aplicar_miniature(const char *implementacion,
                       const char *archivo_entrada,
                       bool verbose, bool frames,
                       const char *carpeta_frames,
...
//devuelve la cantidad de ciclos de llamada a decode_c/asm
unsigned long long int aplicar_decode   (const char *implementacion,
                       const char *archivo_entrada,                       
                       bool verbose);
...
//no la voy a usar, porque voy a medir tiempos unicamente en el procesamiento de frames, igualmente
//voy a mostrar la misma informacion, menos el consumo de ciclos totales incluyendo el proc de opencv.
...
int main( int argc, char** argv ) {
    ...
    if (strcmp(nomb_proceso, "fcolor") == 0) {
        ...
        gc = atoi(argv[argc - 3]);
        rc = atoi(argv[argc - 4]);
        unsigned long long int clocksTotales=0;
        for(int i=0;i<cant_iteraciones;i++){
            // Imprimo info
            printf ( "Procesando...(iteracion %d de %d)\n", i+1, cant_iteraciones);
            printf ( "  Filtro             : %s\n", nomb_proceso);
            printf ( "  Implementación     : %s\n", nombre_implementacion);
            printf ( "  Archivo de entrada : %s\n", nomb_arch_entrada);
		    clocksTotales += aplicar_filtro_color(nombre_implementacion, nomb_arch_entrada,
		                    verbose, frames, carpeta_frames,
		                    rc, gc, bc, threshold);
        }
        if(tiempo==1){
            //unsigned long long int segundosTotales = (unsigned long long int)clocksTotales/CLOCKS_PER_SEC;
            printf("Tiempo de ejecución:\n");
            printf("  # iteraciones                     : %d\n", cant_iteraciones);
            printf("  # de ciclos insumidos totales     : %llu\n", clocksTotales);
            printf("  # de ciclos promedio por llamada : %.3f\n", (float)clocksTotales/(float)cant_iteraciones);
            //printf("  # de segundos insumidos totales     : %llu\n", segundosTotales);
            //printf("  # de segundos promedio por llamada : %.3f\n", (float)segundosTotales/(float)cant_iteraciones);
        }
        ...
        for(int i=0;i<cant_iteraciones;i++){
            ...
            printf ( "  Implementación     : %s\n", nombre_implementacion);
            printf ( "  Archivo de entrada : %s\n", nomb_arch_entrada);
            clocksTotales += aplicar_decode(nombre_implementacion, nomb_arch_entrada,
                        verbose);
        }
        if(tiempo==1){
            //unsigned long long int segundosTotales = (unsigned long long int)clocksTotales/CLOCKS_PER_SEC;
        ...
        for(int i=0;i<cant_iteraciones;i++){
            ...
            printf ( "  Implementación     : %s\n", nombre_implementacion);
            printf ( "  Archivo de entrada : %s\n", nomb_arch_entrada);
            clocksTotales += aplicar_miniature(nombre_implementacion, nomb_arch_entrada,
                    verbose, frames, carpeta_frames,
...
float aplicar_miniature(const char *implementacion,
    ...
    while(1) {
        ...
        //calculo el total de ciclos restando end-start y lo sumo al total de los clocks de todos los ciclos
        unsigned long long int tiempo = end-start;
        clocksTotales+=tiempo;
    ...
    framenum--;//decremento el ultimo frame que era null
    ...
    return ((float)clocksTotales/(float)framenum);
}
                       ...
                       bool verbose)
{
    ...
    unsigned long long int end=0;
    MEDIR_TIEMPO_STOP(end)
    //calculo el total de ciclos restando end-start
    start=end-start;
    /*if(tiempo==1){
        printf("Tiempo de ejecución:\n");
        printf("  # de ciclos insumidos totales     : %llu\n", start);
